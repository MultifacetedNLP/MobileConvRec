...
for index, row in train_raw.iterrows():
    ...
    prev_ = [book['book_name'] for book in prev]
    if len(prev_) > 0:
        previous_interactions.append("<|sep|>".join(prev_)+"<|sep|>")
    else:
        previous_interactions.append(None)
    recommended_book_name.append(row['recommended_book']['book_name'])
    dialog_turns = []
    dialog_index = 0
    found_index = False
    for conv in row['turns']:
        if "COMPUTER" in conv:
            ...
            if (row['recommended_book']['book_name'] in turn) and not found_index:
    ...
    if not found_index: # approximately finding the recommender turn
        for i, dialog_turn in enumerate(dialog_turns):
            if is_approximate_substring(row['recommended_book']['book_name'], dialog_turn):
...
print(len(recommended_book_name))
print(len(recommend_indexes))
df_recommender_train = pd.DataFrame({"user_id": user_id, "previous_interactions":previous_interactions, "recommended_book_name":recommended_book_name, "turns": turns, "recommend_indexes":recommend_indexes})
...
print(len(recommended_book_name))
print(len(recommend_indexes))
df_recommender_validation = pd.DataFrame({"user_id": user_id, "previous_interactions":previous_interactions, "recommended_book_name":recommended_book_name, "turns": turns, "recommend_indexes":recommend_indexes})
...
df_recommender_train['recommended_book_name'] = df_recommender_train['recommended_book_name'].str.lower()
...
df_recommender_validation['recommended_book_name'] = df_recommender_validation['recommended_book_name'].str.lower()
...
for _, row in df_recommender_train.iterrows():
    if row["previous_interactions"] is not None:
        ...
        prompt = bos + "None"
    items_train.append(RecommenderItem(prompt, row["recommended_book_name"] + eos))
...
for _, row in df_recommender_validation.iterrows():
    if row["previous_interactions"] is not None:
        ...
        prompt = bos + "None"
    items_validation.append(RecommenderItem(prompt, row["recommended_book_name"] + eos))
...
training_args = TrainingArguments(
    ...
    save_steps=0.1,#self.cfg.save_steps,
    eval_steps=0.1, #self.cfg.eval_steps,
    save_total_limit=3,
