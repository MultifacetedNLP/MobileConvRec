...
for index, row in test_raw.iterrows():
    ...
    prev_ = [product['product_name'] for product in prev]
    if len(prev_) > 0:
        previous_interactions.append("<|sep|>".join(prev_)+"<|sep|>")
    else:
        previous_interactions.append(None)
    recommended_product_name.append(row['recommended_product']['product_name'])
    dialog_turns = []
    dialog_index = 0
    found_index = False
    for conv in row['turns']:
        if "COMPUTER" in conv:
            ...
            if (row['recommended_product']['product_name'] in turn) and not found_index:
    ...
    if not found_index: # approximately finding the recommender turn
        for i, dialog_turn in enumerate(dialog_turns):
            if is_approximate_substring(row['recommended_product']['product_name'], dialog_turn):
...
for _, row in df_recommender_test.iterrows():
    if row["previous_interactions"] is not None:
        ...
        items_test.append(RecommenderItem(prompt, '<|sep|>' + row["recommended_product_name"] + eos))
