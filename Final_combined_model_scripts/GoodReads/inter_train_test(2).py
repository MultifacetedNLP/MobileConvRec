...
train_raw = pd.read_json("datasets/goodreads/train.jsonl", lines=True)
valid_raw = pd.read_json("datasets/goodreads/val.jsonl", lines=True)
...
for index, row in train_raw.iterrows():
    ...
    prev_ = [(get_first_five_words(product['book_name'])) for product in prev]
    if len(prev_) > 0:
        previous_interactions.append("<|sep|>".join(prev_)+"<|sep|>")
    else:
        previous_interactions.append(None)
    recommended_product_name.append(get_first_five_words(row['recommended_book']['book_name']))
    dialog_turns = []
    dialog_index = 0
    found_index = False
    for conv in row['turns']:
        if "COMPUTER" in conv:
            ...
            if (get_first_five_words(row['recommended_book']['book_name']) in turn) and not found_index:
    ...
    if not found_index: # approximately finding the recommender turn
        for i, dialog_turn in enumerate(dialog_turns):
            if is_approximate_substring(get_first_five_words(row['recommended_book']['book_name']), dialog_turn):
...
test_raw = pd.read_json("datasets/goodreads/test.jsonl", lines=True)
...
apps_training_path = "datasets/goodreads/item_info.csv"
